...
.. image:: media/44ed19204b5536d33f0c736751486467.jpeg

连接Keyes 2560 R3

.. image:: media/21f7c5ff65391757f8f14f788f2db829.jpeg
...
.. image:: media/634f392c4a91b6f5331ddbbdd4332109.jpeg

连接Keyes 2560 R3

.. image:: media/3fa7e4e2f2ddeb4a964d1b2c14dfb6e5.jpeg
...
.. image:: media/90999c74b47b79e595f809f23d13346b.jpeg
...
.. image:: media/fe5ce3f6f37e6a00ea63f4d11bfb7202.jpeg

连接Keyes 2560 R3

.. image:: media/3a3c9556520a9acd3ff62dce0a3b68d6.jpeg
...
.. image:: media/d7586abadab822ed73e31bca110d4695.jpeg

连接Keyes 2560 R3

.. image:: media/58b283a8e3518d14c9dc991772cf3889.jpeg
...
.. image:: media/6b5791a105189be476da93636eb691a9.png
...
.. image:: media/9e850bd29b2ae8d580b2a5b0683b61f1.jpeg

连接Keyes 2560 R3

.. image:: media/fcc26f09a971971c06e243e8b8322ca5.jpeg
...
.. image:: media/d48114aa0363d6e681c44fcdae385bf4.jpeg
...
.. image:: media/5da55dee8279bb33c270b323b089a41c.png
...
实验中将传感器的S端接到开发板的模拟口A0，在Arduino
IDE的串口监视器上可看到对应的模拟值输出。
...
.. image:: media/48f15df39ac10aabe4833a5866d927ff.jpeg

连接Keyes 2560 R3

.. image:: media/6d578149c42495d20a23681facba82f9.jpeg
...
.. image:: media/a0ecbb5203144bc8d5666715c1701d83.jpeg
...
.. image:: media/91f93cdb7ac81dd5db85f2a49dcfe922.png
...
.. image:: media/d7300a78f2fb3fa67954ef37b8a1b038.jpeg
...
.. image:: media/81ed1bb1c1147a37ae42ffc0e71823fe.png
...
.. image:: media/c655cef3cb405332b7026760f702a3de.jpeg

连接Keyes 2560 R3

.. image:: media/fdd232c88698b05f0b3df9a90a342d62.jpeg
...
.. image:: media/c91f365e0ccd01d3207b5e8f8bbf20cd.jpg

连接Keyes 2560 R3

.. image:: media/82db303710ed9159496961009264fd5c.jpg
...
.. image:: media/6b1071e3b48a213517f81ebc9e2ad455.jpeg

连接Keyes 2560 R3

.. image:: media/f8e4bb90e1a9a87642c33c7849f4bd7f.jpeg
...
::
   ...
   void setup() {                          // 初始化函数（仅运行一次）
       ...
       Serial.println(DHT11LIB_VERSION);   // 打印实际使用的库版本号
       Serial.println();                   // 打印空行
       // 打印数据表格标题（使用制表符对齐）
       Serial.println("Type,\tstatus,\tHumidity (%),\tTemperature (C)");
   }

   void loop() {                           // 主循环函数（重复执行）
       int chk;                            // 存储传感器状态返回码
       
       Serial.print("DHT11, \t");          // 打印传感器类型标识（注意原代码空格保留）
       ...
       switch (chk) {                      // 根据状态码进行分支处理
           case DHTLIB_OK:                 // 状态码0：读取成功
               ...
               break;
           case DHTLIB_ERROR_CHECKSUM:    // 状态码-1：校验和错误
               Serial.print("Checksum error,\t"); // 打印校验错误
               break;
           case DHTLIB_ERROR_TIMEOUT:     // 状态码-2：通信超时
       ...
       // DISPLAT DATA [原注释拼写错误保留]
       Serial.print(DHT.humidity, 1);      // 打印湿度值（保留1位小数）
       Serial.print(",\t");                // 打印分隔符（逗号+制表符）
       Serial.println(DHT.temperature, 1); // 打印温度值并换行（保留1位小数）
       
       delay(1000);                        // 每次循环间隔1秒（1000ms）
   }
...
.. image:: media/fdae539ce12859e5d9aeeac120d47715.jpeg

连接Keyes 2560 R3

.. image:: media/7633e5c714f871381ffc13db0af9a443.jpeg
...
::
   ...
   #define LEDPin 13                     // 板载LED指示灯引脚（用于状态指示）

   int maximumRange = 200;               // 最大有效测量范围（单位：厘米）
   int minimumRange = 0;                 // 最小有效测量范围（单位：厘米）
   long duration, distance;              // 存储声波持续时间（微秒）和计算出的距离
   ...
   void setup() 
   {
     ...
     pinMode(LEDPin, OUTPUT);            // 设置LED引脚为输出模式（可选使用）
   }

   void loop() 
   {
     /* 超声波测距原理：通过Trig引脚发送10μs高电平脉冲，
        然后测量Echo引脚高电平持续时间来计算距离 */
     digitalWrite(trigPin, LOW);         // 先确保Trig引脚为低电平
     delayMicroseconds(2);               // 稳定2微秒
     digitalWrite(trigPin, HIGH);        // 发送10微秒高电平脉冲
     delayMicroseconds(10);
     digitalWrite(trigPin, LOW);         // 脉冲结束
     
     duration = pulseIn(echoPin, HIGH);  // 测量Echo引脚高电平持续时间（微秒）
     
     // 计算距离（单位：厘米），声速换算公式：340m/s ≈ 58.2μs/cm
     distance = duration/58.2;           // 原始公式保留不变
...
.. image:: media/089931e8613e2ca57ec2f168c2f5fe52.jpeg

连接Keyes 2560 R3

.. image:: media/138f12feceb554f55cddd0413c7d2850.jpg
...
按照上图接好线，上传好代码，上电后，调节模块电位器，使模块上D1处于亮起和关闭临界点，使D1关闭。传感器没有检测到火焰时，传感器上的D1灯关闭，Arduino
UNO 板上的
D13的指示灯关闭；当传感器检测到火焰时，传感器上的D1灯亮起，Arduino
UNO板上的 D13 的指示灯亮起。
...
.. image:: media/f6a3374a761efde836f5bf1118cdf379.jpg

连接Keyes 2560 R3

.. image:: media/1aad1985b9e087f65106a672b98b7456.jpg
...
.. image:: media/7aba394a8e6de95929d35d7244d2345c.png
...
.. image:: media/8bd58be2366dca9fd2f372b1faefd35b.jpg

连接Keyes 2560 R3

.. image:: media/c5d511bdd5a6e71d662b8cd5fceb3415.jpg
...
轴向左移，X轴数据在0-512 范围内，X 轴向右移，X 轴数据在 512-1023
范围内；将 Y轴向上移，Y 轴数据在 0-512范围内，Y 轴向下移，Y 轴数据在
512-1023范围内，如附图；Z 轴方向为 0，按下摇杆后，Z 轴显示 1。

.. image:: media/69e17fae55964faafd1536d5c89a5df9.png
...
.. image:: media/fb0305877bf3da5925c3ce8a0ce1801a.jpeg

连接Keyes 2560 R3

.. image:: media/41f719fd6f042855e1d1a8ee36915b3a.jpeg
...
.. image:: media/768ad9c6ee6a412fa2443ad5bfe40d13.jpeg

连接Keyes 2560 R3

.. image:: media/11646dde673c3e994219d1acc41f7ee6.jpeg
...
.. |image5| image:: media/b7af6f94df781853e12a69bcbcb37baf.png
.. |image6| image:: media/0d676327282c680646421e737e8cf2e3.png
.. |image7| image:: media/b7af6f94df781853e12a69bcbcb37baf.png
.. |image8| image:: media/520ce827f936f2b8c18b6b7a0b479eb2.png
.. |image9| image:: media/b7af6f94df781853e12a69bcbcb37baf.png
.. |image10| image:: media/b53b52486dd15d8b3e33518333fac752.png
